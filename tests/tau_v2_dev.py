...
def load_graph(path):
    ...
    if ext == ".csv":
        ...
        w = df["weight"].astype(float).tolist() if has_w else None

        # map node ids -> 0..n-1 (stable order)
        import numpy as np
        nodes = pd.Index(src).append(pd.Index(tgt)).unique().tolist()
        idx = {n: i for i, n in enumerate(nodes)}
        edges = [(idx[u], idx[v]) for u, v in zip(src, tgt)]

        ig_graph = ig.Graph(n=len(nodes), edges=edges, directed=False)
        if has_w:
            ig_graph.es["weight"] = w

    else:
        # adjacency-list path (original behavior)
        nx_graph = nx.read_adjlist(path)
        mapping = dict(zip(nx_graph.nodes(), range(nx_graph.number_of_nodes())))
        nx_graph = nx.relabel_nodes(nx_graph, mapping)
        ig_graph = ig.Graph(
    ...
    # sanity checks
    if ig_graph.vcount() < 2 or ig_graph.ecount() < 1:
        raise ValueError(f"Loaded graph too small: |V|={ig_graph.vcount()} |E|={ig_graph.ecount()}")
    return ig_graph
